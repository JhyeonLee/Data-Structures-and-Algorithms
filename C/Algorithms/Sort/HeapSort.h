...
// orderType: Ascending(1) Descending(2)
SequentialHeap* inPlaceHeapSort_Insertion_Recursive(int orderType) {
    int N; int type;
...
SequentialHeap* inPlaceHeapSort_Insertion_nonRecursive(int orderType) {
    int N; int type;
    printf("Acceptable Size and Type for Sequential Heap? Min Heap(1) Max Heap(2)"); scanf("%d %d", &N, &type);
    SequentialHeap* H = SequentialHeap_initialize(N, type);
    // Phase 1: Build Heap
